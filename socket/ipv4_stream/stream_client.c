...
void chat_func(int server_fd)
{
	...
	memset(recvbuff, '0', BUFF_SIZE);
        ...
        if (numb_write == -1)  exit(EXIT_FAILURE);
        ...
        if (numb_read < 0)  exit(EXIT_FAILURE);
...
int main(int argc, char *argv[])
{
    ...
    if (inet_pton(AF_INET, argv[1], &serv_addr.sin_addr) == -1) exit(EXIT_FAILURE);
    ...
    /* Kết nối tới server*/
    if (connect(server_fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == -1) exit(EXIT_FAILURE);
    chat_func(server_fd);
